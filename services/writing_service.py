...
async def generate_outline_mapping(payload: dict) -> dict:
    service = get_service_bundle().writing
    response = await service.outline_mapping(OutlineMappingRequest(**payload))
    return response.model_dump()
...
async def heuristic_start_stream(payload: dict):
    ...
    graph, input_messages = _build_stream_graph(
        ...
        image_urls=_extract_image_urls_from_text_list(payload.get("textList") or []),
    )
    async for line in graph_to_ndjson_tokens(graph, {"messages": input_messages}):
        yield line
...
def _build_stream_graph(
    *, system_prompt: str, user_text: str, messages: list, image_urls: list | None = None
) -> tuple[StateGraph, list]:
    model = build_chat_model(streaming=True)
    lc_messages = build_messages(
        system_prompt=system_prompt, user_text=user_text, messages=messages, image_urls=image_urls
    )
...
def _extract_image_urls_from_text_list(text_list: list) -> list:
    return extract_image_urls_from_text_list(text_list)
