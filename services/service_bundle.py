...
from services.agents.mapping import MappingAgent
...
class WritingServiceImpl(WritingService):
    ...
    async def outline_mapping(self, req: OutlineMappingRequest) -> OutlineMappingResponse:
        mapping = self.mapping_agent.agenerate_mapping(req.model_dump())
        return OutlineMappingResponse(sessionId=req.sessionId, neighbors=mapping)
...
def build_service_bundle() -> ServiceBundle:
    ...
    return ServiceBundleImpl(
        writing=WritingServiceImpl(outline_agent, mapping_agent, heuristic_agent, restruct_agent),
